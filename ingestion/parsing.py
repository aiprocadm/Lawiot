...
# Раздел римской цифрой, опц. с дробным суффиксом: «Раздел I. …» / «Раздел III.1. …»
# (ЖК РФ: III.1, III.2). Суффикс «.N» обязателен в захвате — иначе III.1/III.2
# схлопывались в «III» → один якорь razdel-iii на три раздела.
# Ключевое слово БЕЗ re.IGNORECASE (только «Раздел»/«РАЗДЕЛ», не строчное
# «раздел»): заголовки всегда с прописной, а строчная «раздел IV Указа…» в прозе
# (ТК РФ, ст. 423) иначе ловилась как фантомный раздел. Ср. ARTICLE_RE («Статья»).
SECTION_RE = re.compile(r"^(?:Раздел|РАЗДЕЛ)\s+([IVXLCDM]+(?:\.\d+)?)\.?\s*(.*)$")
# Глава арабской цифрой («Глава 1.» / «Глава 12.1.») ИЛИ римской, опц. с суффиксом
# («Глава I.» — 10-ФЗ о профсоюзах; «ГЛАВА V.1» — ЗК РФ, V.1..V.7). Суффикс «.N»
# у римской ветки обязателен (как у арабской), иначе V.1..V.7 → «V». Без
# re.IGNORECASE (см. SECTION_RE): только «Глава»/«ГЛАВА».
CHAPTER_RE = re.compile(r"^(?:Глава|ГЛАВА)\s+(\d+(?:\.\d+)?|[IVXLCDM]+(?:\.\d+)?)\.?\s*(.*)$")
