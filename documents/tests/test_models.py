...
@pytest.mark.django_db
def test_article_anchor_deduplicated_within_redaction():
    # БК РФ: две настоящие «Статья 242.1» (утратившая силу + действующая) делят
    # номер. UniqueConstraint(redaction, anchor) запретил бы вторую строку —
    # save() детерминированно вешает суффикс -2/-3 вместо падения IntegrityError.
    red = make_redaction()
    first = make_article(red, number="242.1", order=1)
    second = make_article(red, number="242.1", order=2, title="Общие положения")
    third = make_article(red, number="242.1", order=3)
    assert first.anchor == "st-242-1"
    assert second.anchor == "st-242-1-2"
    assert third.anchor == "st-242-1-3"


@pytest.mark.django_db
def test_article_anchor_dedup_is_scoped_to_redaction():
    # Один номер в РАЗНЫХ редакциях сохраняет чистый якорь — суффикс не «протекает»
    # между редакциями (ограничение и дедуп действуют в пределах одной редакции).
    doc = make_document()
    r1 = make_redaction(doc, redaction_date=date(2023, 1, 1))
    r2 = make_redaction(doc, redaction_date=date(2024, 1, 1))
    assert make_article(r1, number="242.1").anchor == "st-242-1"
    assert make_article(r2, number="242.1").anchor == "st-242-1"


@pytest.mark.django_db
def test_article_hierarchy_parent_children():
    red = make_redaction()
    chapter = make_article(red, kind=Article.Kind.CHAPTER, number="13", title="Глава 13", order=1)
...
def test_article_valid_deep_hierarchy_accepted_by_clean():
    ...
    chapter = make_article(red, kind=Article.Kind.CHAPTER, number="13", parent=section, order=2)
