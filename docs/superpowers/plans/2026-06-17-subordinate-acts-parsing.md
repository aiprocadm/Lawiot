...
**Конкретные сценарии для проверки на живом образце** (из финального ревью фичи — синтетические тесты их НЕ покрывают):

1. **`POINT_RE` ложные пункты из перечней.** `^(\d+(?:\.\d+)*)\.\s+\S` поймает легитимный перечень в теле пункта («1. молоко;») или нумерованные строки таблиц как POINT-узлы (и они «украдут» следующие строки тела). Гард против `2.5 ставки` (десятичные в прозе) НЕ ловит маркеры-перечни `N.`. Проверить на живом доке, не всплывают ли ложные пункты.
2. **`APPENDIX_RE` переловы.** `Приложени\w*` ловит ЛЮБУЮ строку, начинающуюся с «Приложение…», включая ссылки в тексте («Приложением N 2 установлено…», «Приложение к приказу не применяется»). Номер `(\d+)?` опционален → даже «Приложения не требуется.» станет контейнером. Убедиться, что заголовки приложений в живом доке — отдельные строки, а не начала предложений в теле.
3. **Размещение штампа `УТВЕРЖДЕН[АОЫ]?`.** Штамп обычно в правом верхнем блоке приложения; после `html_to_text` проверить, что он лёг на отдельную строку и что в `title` (group 2) не попало «постановлением Правительства РФ от …» вместо реального заголовка.
4. **Продолжение пункта через абзацы и подпункты «а)»/«б)».** Они должны падать в `body` (хорошо) — убедиться, что ни один не начинается с паттерна «цифра-точка».
5. **Нарушение порядка нумерации** (`1.1.1` раньше `1.1`). `orphan_subpoint` падает в контейнер — проверить, что реальный док не даёт визуально-кривую вложенность из-за непоследовательной нумерации.
6. **Переиспользование `SECTION_RE`/`CHAPTER_RE` внутри приложений.** `SECTION_RE` ловит ТОЛЬКО римские цифры; некоторые приложения нумеруют «Раздел 1» (арабская) — такие НЕ матчатся и провалятся. Проверить стиль нумерации разделов в живом доке.

**Известный Minor (вынести в follow-up при многоприложенческих актах):** якорь пункта (`p-<номер>`) не уникален между приложениями — нумерация пунктов рестартует в каждом приложении, поэтому «п. 1» в Приложении 1 и в Приложении 2 дадут одинаковый `p-1` (TOC/`id` коллизия — браузер прыгнет к первому). Преекзистинг-класс (якоря и раньше не были уникальны), но для пунктов вероятнее. Решение при необходимости: подмешать `order`/номер приложения в якорь пункта.
